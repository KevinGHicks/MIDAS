...
pool_data %>%
  ...
  dplyr::mutate(pool = glue::glue("pool {pool}")) %>%
...
## Infer Metabolite-Protein Interactions (MPIs) from Corrected Fold-Changes
...
ggplot(enrichment_statistics, aes(x = p_value)) +
  ...
  ggtitle("P-value distributions for all tested MPIs")
```

## Summarize FDR-controlled MPIs
...
```{r scatter_plot, fig.height = 10, fig.width = 10}
...
```{r example_metabolite_dist, fig.height = 10, fig.width = 10}
...
```{r save_results}
midas_mpis = enrichment_statistics %>%
  dplyr::select(-met_data, -met_mean, -met_sd)

# save to packages with:
# this data can be accessed with data(midas_mpis)
# usethis::use_data(midas_mpis, overwrite = TRUE)

util_pretty_khead(midas_mpis %>% filter(q_value < 0.1), nrows = 5, caption = "Example significant MPIs")
```
